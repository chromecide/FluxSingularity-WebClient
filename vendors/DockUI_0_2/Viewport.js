...
Ext.define('DockUI.Viewport', {
	...
	loadViewport: function(record){
		...
		if(leftBar.Applications){
			for(var i=0;i<leftBar.Applications.length;i++){
				this.addSidebarApplication('left', leftBar.Applications[i]);
			}
		}
		if(rightBar.Title){
			var rb = Ext.getCmp('dockui-viewport-rightbar');
            ...
            items:[
            	{
	                ...
	                items: this.settings.topBar.items
	            },
	            {
	...
	addSidebarApplication: function(location, appCfg){
		var bar = null;
		if(location=='left'){
			bar = Ext.getCmp('dockui-viewport-leftbar');
		}else{
			bar = Ext.getCmp('dockui-viewport-rightbar');
		}
		var app = Ext.create(appCfg.Name);
		var appPanel = {
        	xtype:'panel',
        	layout:'fit',
            draggable:true,
